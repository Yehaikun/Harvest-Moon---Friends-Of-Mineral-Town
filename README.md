...
| 第 2 阶段 | 脚本索引与触发链路 | 脚本 ID、ROM 偏移、slot 大小、`Proc016` 传送点、实体脚本表、ROM 数据区引用、NPC 日常对话映射、入口触发链文档 | ✅ 已完成：gUnk_080F1FC0 解码（104/266 有效）、40 个 NPC 日常对话脚本映射完成、入口触发链（地图瓦片→实体类型→gUnk_080F1FC0→脚本）已文档化 |
...
| 第 4 阶段 | 数据补丁器与校验工具 | 文本长度检查、指针/偏移检查、脚本编译输出大小检查、mary 坏脚本黑名单（`tools/pre_build_checks.py`、`make check-all`） | ✅ 已完成：slot 大小检查、指针表完整性检查、1328 脚本批量测试（1296 OK, 32 PARTIAL 已列入黑名单）、`make check-all` 一键验证 |
| 第 5 阶段 | 地图与场景数据链路 | 地图 ID、出生点、传送点、地图块/碰撞线索、DataCrystal/论坛资料交叉表（`docs/references/map_data_guide.md`） | ✅ 已完成文档化。地图碰撞格式解码需要深入 MP 瓦片格式的事件层，作为后续反编译工作 |
| 第 6 阶段 | 文本与内容数据工具评估 | HMMT、DataCrystal ROM map、khadim 资料、CodeBreaker 地址的可用性测试文档（`docs/references/content_tool_evaluation.md`） | ✅ 已完成：8 种工具的可用性评估表，确定 .mary 脚本/物品价格/物品名可改，地图碰撞/新内容需反编译 |
